...
### Apple — SwiftUI Clustered Workspace (watch · phone · iPad · Mac · MacBook · MacBook Pro · AR glasses)

The `apple/` directory contains the SwiftUI build for Xcode — an **HD / 3D GUI clustered workspace** where the **watch, phone, iPad, Mac, MacBook, MacBook Pro, and AR glasses each run a different function at the same time**, connected through **iCloud** (key-value store). The UI is a **tabbed shell with SF Symbol icons for every device and tab**, plus a full **HD / 3D Display tab** — devices that support spatial 3D (MacBook Pro, AR glasses) render with a depth effect.

- **`apple/Sources/AicodeXCore/`** — platform-independent cluster logic: `ClusterDevice` (each device kind has a distinct function, icon, and `DisplayProfile`), `DisplayProfile` (HD label + 2D/3D rendering capability), `AppTab` (all tabs with their SF Symbol icons), and `ClusterCore` (progress clamping, aggregate progress, iCloud keys). Builds and tests everywhere — `swift test` runs on Linux CI too.
- **`apple/Sources/AicodeXApp/`** — the **buildable SwiftUI app target**: the `@main` entry point, the tabbed `ContentView` (Cluster, Devices, Display, Settings tabs), the HD/3D `DeviceDisplayCard` (per-device live progress, HD label, spatial depth effect), and `ClusterStore` bridging `AicodeXCore` to `NSUbiquitousKeyValueStore` for realtime cross-device sync. The sources use **conditional compilation** (`#if canImport(SwiftUI)`), so `swift build`/`swift run` succeed cross-platform while the full SwiftUI UI is compiled on Apple platforms (and via Xcode for the App Store).
